...
class MediastackProvider(BaseProvider):
    """
    Mediastack news API provider.
    Handles rate limits, quotas, and API errors gracefully without raising exceptions.
    """

    provider_name = ProviderName.MEDIASTACK

    def __init__(self, query: str = "news") -> None:
        self.query = query
        self.api_key = get_settings().mediastack_api_key
            ...
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                return [
                    RawDocument(
                        ...
                        source_name=a.source_name or "Mediastack",
                        metadata={"search_query": self.query},
                    )
                    for a in articles
                ]
        except Exception as e:
            logger.warning("mediastack_fetch_latest_failed", error=str(e))
            return []
    ...
    async def search(
        ...
        *,
        ...
        if not self.api_key:
            logger.debug("mediastack_no_api_key", msg="MEDIASTACK_API_KEY not set")
            return []
        ...
        try:
            response = await http_client.get(f"{MEDIASTACK_BASE}/news", params=params)

            if response.status_code in (429, 403, 401, 402):
                logger.warning("mediastack_rate_limit_or_quota_exceeded", status_code=response.status_code)
                return []
            if response.status_code != 200:
                logger.warning("mediastack_api_non_200", status_code=response.status_code)
                return []

            data = response.json()
            if "error" in data:
                logger.warning("mediastack_api_error_response", error=data["error"])
                return []
            ...
            logger.info("mediastack_search_done", query=query, count=len(articles))
            return articles
        except Exception as exc:
            logger.warning("mediastack_request_failed", query=query, error=str(exc))
            return []
